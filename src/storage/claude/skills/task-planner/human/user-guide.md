...
- Claude Code Kanban set up for your project
- Git repository with a remote

## How It Works

```
You (in Claude Code)          Plan file on disk          Claude Code Kanban
     |                       |                         |
     |--- "plan STAX-42" -->|                         |
     |    writes plan ------>|                         |
     |                       |--- cards created ------>|
...
Claude Code plans. Kanban executes. The plan file is the only bridge.
...
Or just say `plan STAX-42` and Claude Code will ask you to classify the tier.

### Step 2: Claude Code Fetches and Presents the Ticket

Claude Code reads the Jira ticket and shows you:
...
### Step 4: Claude Code Writes the Plan

Claude Code creates `plans/plan-STAX-42-small.md` in your project root:
...
If a ticket involves UI changes, Claude Code will ask:
...
| Tier   | Trigger example       | Planning time | What Claude Code does                 |
| ------ | --------------------- | ------------- | ------------------------------- |
...
| Plan templates | `~/.claude/skills/task-planner/dependencies/templates/` |
| Mode docs      | `~/.claude/skills/task-planner/modes/`                  |
...
- Plan multiple tickets in one Claude Code session. Each plan kicks off independently in Kanban.
