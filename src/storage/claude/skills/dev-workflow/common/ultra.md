...
3. Transition the ultra ticket to "in review" via the `jira` skill, and record the spec's repo path on it — the spec is the deliverable, its PR is now open, and a [chain](auto-chain.md) later reads that path off this ticket.
