...
4. **Report.** One report for the chain, not one per component. Each component's exit report was written for a reader who was not there — collapse them into a single [exit report](../templates/exit-report.md) for the whole spec: what the user can now do, what to test on the integration branch, which component stopped and why, which never started, every check that came back red on that first continuous-integration run, and every assumption, deferral, and unfixed finding the components accumulated along the way.
