...
3. **Hand the user the goal, and wait for it.** The waves are derived, so the end state can now be named exactly, and nothing has been created yet — a user who walks away from this message leaves no branch, no worktree and no ticket behind. Draft one measurable end state whose evidence lands in the transcript, per [auto mode](../SKILL.md#auto-mode), and give it to them ready to paste:
