...
1. **Arm the guard** for the whole chain — `~/.claude/hooks/auto-run-guard.sh start <SPEC-TICKET>`. Disarming is the last tool call of the chain, before the hand-back report and before any halt. The managers are subagents and are not covered by it; they fire `SubagentStop`, which this run must leave alone, and they are told not to arm one of their own.
   ...
   **The guard is a negative control, so its failure is silent** — an armed marker that never fires is indistinguishable from one correctly permitting a stop, and one has sat armed through a dozen hand-backs without objecting once. The stopping rule is the goal, and it has to be stated in a form its evaluator can use: **one measurable end state whose evidence appears in the conversation.** That evaluator reads conversation text and cannot run a command or open a file, so a condition like *do not let managers cut corners* is judged against nothing but your own progress reports, which are the least reliable input available. Set the check-in to roughly ten minutes — `CLAUDE_CODE_GOAL_CHECKIN_MINUTES=10` — rather than the default, which starts at thirty and doubles to two hours exactly as a stall lengthens, and re-arm after any interruption — a goal does not survive the process exiting and nothing announces its absence.

   **Arm a wake-up now, not when you need one.** A usage limit is the one failure this session provably cannot recover from at the moment it arrives: the block is account-wide, so nothing can be scheduled once it lands. One such outage cost three and a half hours *after* the limit had already reset, with nothing listening. Before the first dispatch, set a periodic check on a cadence longer than a wave whose only job is to see whether anything is still live and re-dispatch if not.
