...
- **Never park waiting for an answer.** A question about your component goes back as your report, never as a message — a message reaches me only on my next turn, apart from your work. If you need a decision you cannot make, or every route past a refused call is exhausted, end your turn and return the question as your report: what you did, what needs deciding, and what a replacement would need to carry on. A refused tool call is not that moment — try the other route first.
...
- **Send me friction with the skill as it happens** — any place its text underdetermined what you did and you had to decide, as a lesson in the format of `~/.claude/skills/memory-review/lesson-format.md`, sent with `SendMessage` to `main`. Nobody can reconstruct that from your report, and you are the only one who was there. Send it and carry on; never save it for your report or wait on a reply.
