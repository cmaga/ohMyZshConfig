...
Its check-in fires on an idle timer as well as at turn end, so it can start a turn and not only refuse to end one. That is the whole mechanism: the failure this guards against is a run going quiet, never a run stopping too readily. Two things it cannot do — it does not survive the process exiting, and some errors clear it silently, with nothing announcing either. Neither is visible from inside the run, so a run that notices any interruption asks the user to re-arm.

**The goal is the only thing holding an ordinary run open. Schedule nothing alongside it.** A periodic self-check costs a turn every time it fires and, on a run that is waiting on the user rather than stalled, every one of those turns re-reports the same thing. Only [the chain](common/spec-run.md) arms an out-of-process wake-up, because only a chain runs long enough to outlive a usage block — and it takes that wake-up down at hand-back.
...
1. **Task tracking.** Seeded once the routing call above is made. Load the tools first via `ToolSearch` with `select:TaskCreate,TaskUpdate` — both are deferred, so calling them cold fails. If they do not resolve, `CLAUDE_CODE_ENABLE_TODO_TOOLS` is missing from the `env` block of `~/.claude/settings.json` — current models suppress the task tools without it. Adding it as `"1"` restores them mid-session, so tell the user in one line and retry the `ToolSearch` call once. If they still do not resolve, skip this item, mark step transitions in prose instead, and carry on with the workflow. Otherwise call `TaskCreate` once per item for each of these exact items to seed the status list, then keep it current with `TaskUpdate` — mark the prior item `completed` and the next `in_progress` as you move through the flow. This list is the user's at-a-glance status surface, not narration; maintain it even when responses are otherwise terse.
   ...
   6. Implementation
   7. Exit
2. **Lever aliases.** Read `~/.claude/skills/optimize-usage/lever-state.json` and bind the `kind: "skill"` levers this workflow uses — `RESEARCH_FANOUT_MODEL`, `CODE_FANOUT_MODEL`, `MECHANICAL_WORKER_MODEL`, `JUDGMENT_WORKER_MODEL`, `ESCALATION_MODEL`, and `PARENT_FANOUT_MODEL` (every agent this run dispatches that no other alias covers) — to the all-caps form of their keys. A value of `inherit`, a key absent from the file, or a missing file all mean no pin: omit the `model` opt wherever that alias is used and let the agent's own default stand. Never invent a value for an absent key. If the values fall out of context later (long session, compaction), re-read the file rather than trusting memory. Prefix every dispatched agent's `description` with the alias it carries and that alias's value, as `<alias>(<model>/<effort>): <what it does>` — `code-fanout(opus): seam check on the intake path`. Name only the levers that apply. The description is recorded for every agent and the model and effort are recorded for neither, so the prefix is the only way to audit afterwards whether an agent ran on the lever it was supposed to.
...
Every step below is labelled **internal** or **user-facing**, and the label is binding. An internal step produces no post and never waits — and prints nothing between its tool calls: no running commentary, no findings as they land, no note of what you are about to check next. The task list is the status surface, and prose during an internal step is narration the brief then has to compete with. Its output is working context for the steps after it, and pausing there hands back mid-thought on something the user was never asked to answer. Only a user-facing step is entitled to end a turn. Every message that waits on the user — a brief, a presentation, a scaffold review, a reply inside any of them, a report that stops the run — is at most four sentences of plain words, plus anything it has to show (a file list, a `/goal` line), then each thing that needs their answer, numbered. Detail is what they ask for next.
...
1. Dispatch `scoping-agent` with the ticket and Step 1's restatement. It returns the case file the rest of the workflow consumes: problem in code, docs, history, and whether this is needed. Read it; do not repeat its reading.
   - If the case file says this should not be done, stop here and report back to the user with a simple high level explanation. Before reporting, undo what Step 1 did: move the ticket back to its previous status and unassign it. Leaving it In Progress tells the board someone is working on it. Do not transition it to anything opinionated — Blocked or Won't Do is the user's call, made with the reason in hand.
...
2. Each component's ticket re-enters this workflow at Step 1 at its own tier — its own worktree, merging locally into the spec's integration branch, ending at [Exit](common/exit.md). No component opens a pull request: the branch is local and the whole spec goes up once. Which components build at the same time is derived from their declared edges, not written down — walked by the user or by [the chain](common/spec-run.md). A chain finishes the spec itself ([hand-back](common/spec-run.md#hand-back)). Walked by hand there is no chain to do it, so the run that finishes the spec's last component says so in its exit report — read the ultra ticket's linked component tickets to know that you are it, since nothing else tells you and names what closing the spec takes: reading the spec against the finished branch the way [hand-back](common/spec-run.md#hand-back) step 1 does — the per-component checks never made that pass — then pushing the integration branch — nothing has pushed it until now — then a PR from it into the project's own base branch, and a worktree on it to run it from. That PR is the first time continuous integration sees any of the spec. Do both when the user asks — never unasked, since the spec reaching the base branch is their call.

## Critical Rules

- Friction with **this skill** is recorded when it happens, never at the end. Two triggers: the user correcting how the run works rather than what it builds, or text that underdetermined something you then had to decide. File it as a lesson in auto memory, per [lesson format](../memory-review/lesson-format.md), the moment either fires — mid-run, never saved for a report, discussed, or waited on.
  - **Every process correction the user makes is one, including where the lapse was yours.** A rule this skill states and you did not apply at the moment it mattered is a placement defect until a reader holding every other lesson says otherwise. Writing it off as your own slip is how the defect survives. Fault is never yours to rule on here; the lesson is.
  - A slip you caught yourself, with nobody correcting you, is **not** a lesson. Their correction is the event: without one there is nothing separating text that failed from a run that stumbled, and a run logging its every stumble fills memory with what no reader can act on.
  - The test is mechanical: **if your next tool call exists because the user just told you to make it, and what they told you is about how the run works rather than what it builds, the lesson goes in that same message.** A correction you can obey in one command is still a correction — quick ones read as instructions to carry out and are the lessons that never get written, which is exactly why those defects never get fixed.
  - A correction about the code, and an outcome the text told you to expect, are both not lessons — the first is the discovered-issue rule above, the second is the skill working.
...
- Discovered-issue routing: a problem found mid-flow (scope, scaffold, planning, implementation, or review) is routed when found, never parked as prose.
  - **Fix it. That is the default, and the bar for leaving instead is high.** The one question is whether the fix can ride this branch — not whether it is "related", not whether it is in scope, not whether it is what the ticket asked for. You are already in the code, already hold the reasoning, and already have the gates green: this is the cheapest moment the fix will ever have. Deferring converts twenty minutes into a ticket, a future take, a fresh worktree and a second full-suite run, and the user pays for all four.
  - Leave it only when the fix genuinely cannot ride along — a different subsystem, a design decision the user owns, or a scope that would double the diff. Then search first and propose it with your recommendation, saying plainly why it could not ride. A proposal you cannot justify that way is a fix you should have made.
  - **A tracker full of proposals is the failure mode, not the format working.** Every one of them asks the user to make a decision you were better placed to make, and it reads as work handed back. If you are seeding a third, stop and re-ask whether any of them could simply have been fixed.
...
- Nothing dispatched ever parks waiting for an answer. A question about the work goes back as the agent's report, never as a message: `SendMessage` to `main` reaches the main session only on its next turn and carries nothing of the work, while an ended turn returns the question as the agent's result. When you need a decision you cannot make, **end your turn and return the question as your report**. Messages to `main` are for what is not the work, such as a [skill lesson](../memory-review/lesson-format.md). Return what you did, what you need decided, and what a replacement would need, so the work survives whether you are resumed or replaced.
- **Three levels, and the worker level is the floor.** A session dispatches managers, a manager dispatches workers, and a worker dispatches nothing. A completion reaches its spawner one level down and no further: deeper than that it surfaces to the main session, so the agent that asked never receives it and waits forever on a result already delivered to somebody else. Say it in every dispatch prompt, in the second person — *anything you spawn reports to me, not to you; do the work yourself* — and make each level say it to the one below. Where a job is genuinely too large for one agent, the level that can hold the results splits it and dispatches the pieces itself.
  ...
  - **When an agent goes quiet, read its status with `ListAgents` before believing anything.** Silence is not evidence: a long review or a deep read is silent by nature. A finished agent whose result never reached its waiter was mis-addressed — check your queue, relay it in full, and say why it is arriving from you. Replace only an agent that is no longer running and delivered nothing.
