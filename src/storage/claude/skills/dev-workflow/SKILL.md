...
- Friction with **this skill** is recorded when it happens, never at the end: the user correcting how the run works rather than what it builds, or text that underdetermined something you then had to decide. One file per entry per [friction](common/friction.md), written mid-run and never reported, discussed, or waited on. A correction about the code is not one — that is the discovered-issue rule above.
- Never automatically merge a PR into the base branch. The user merges or asks you to merge; auto mode is that ask. Merging a component into a spec's integration branch is the one exception, and only ever for whoever owns the component's ticket: a chain merges its components, a lone auto run on a component ticket merges its own, and a component dispatched inside a chain merges nothing — its parent does. That merge is local and opens no pull request. The user's merge is the single one at the end that takes the whole spec to the base branch.
...
- **An agent you dispatched must not fan out again.** A subagent's completion reaches its spawner one level down and no further — deeper than that the result surfaces to the main session instead, so the agent that asked for the work never receives it and reports the work as never returned. Whoever dispatches a fleet must be the one holding its results. Say so in every dispatch prompt: gather your own evidence, and do not spawn agents of your own. Where a job is genuinely too large for one agent, the level that can hold the results splits it and dispatches the pieces itself.
  - This binds the reviewer too. A review gate that fans out to per-dimension reviewers loses every one of their findings, and returns a verdict over the ground it happened to cover itself — which reads as a pass over the whole diff. If a review needs more than one agent, the parent runs those agents.
